...
import { runOutlineTask } from "@/lib/jobs/task-actions";
...
export async function POST(_request: Request, context: { params: Promise<{ taskId: string }> }) {
  ...
  const queued = enqueueTaskJob(taskId, "outline", runOutlineTask);
  const updated = await prisma.paperTask.update({
    where: { id: taskId },
    data: queued.alreadyQueued
      ? {}
      : { status: "queued_parse", progress: Math.max(task.progress, 10), errorMessage: null },
    include: { paragraphs: { orderBy: { index: "asc" } } }
  });
